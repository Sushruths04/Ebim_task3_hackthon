...
# 2026-08-21: this module used to call `room_scene.franka_urdf_path(...)`.
# `scene_robot_room_keyboard` only re-exports `asset_path` from
# `path_utils`, never `franka_urdf_path`, so that call raised
# AttributeError at argument-parse time -- i.e. this entry point could not
# start at all. Import it from where it actually lives rather than
# re-exporting it through an unrelated scene module.
from path_utils import franka_urdf_path  # noqa: E402
...
def run(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print("RMPFLOW_RUN_FAILED -- traceback follows", flush=True)
        traceback.print_exc()
        sys.stdout.flush()
        sys.stderr.flush()
        raise
